...
import useOutlineEditorDataAdapter from "~/editor/adapters/useOutlineEditorDataAdapter";
import env from "~/env";
...
const LegacyEditor = lazyWithRetry(() => import("~/editor"));
const PackagedEditor = lazyWithRetry(() => import("@outline-editor/core"));
...
function Editor(props: Props, ref: React.RefObject<SharedEditor> | null) {
  ...
  const defaultHostAdapter = useOutlineEditorDataAdapter({ documentId: id });
  const hostAdapter = props.hostAdapter ?? defaultHostAdapter;
  const usePackagedEditor = env.ENABLE_PACKAGE_EDITOR === "true";
  const LazyLoadedEditor = usePackagedEditor ? PackagedEditor : LegacyEditor;
  ...
  const handleUploadFile = React.useCallback(
    async (
      ...
      if (props.uploadAdapter) {
        return props.uploadAdapter.uploadFile(file, uploadOptions);
      }
