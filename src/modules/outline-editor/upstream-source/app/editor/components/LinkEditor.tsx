...
import type { EditorDocumentSearchResult } from "@shared/editor/types/EditorDataAdapter";
...
type SearchResult = EditorDocumentSearchResult;
...
const LinkEditor: React.FC<Props> = ({
  ...
  const [adapterResults, setAdapterResults] = useState<SearchResult[]>([]);
  const { hostAdapter } = useEditorHost();

  const trimmedQuery = query.trim();
  const results: SearchResult[] = adapterResults;

  useEffect(() => {
    if (!hostAdapter?.searchDocuments || !trimmedQuery) {
      setAdapterResults([]);
      return;
    }
    ...
    void hostAdapter
      .searchDocuments({ query: trimmedQuery, limit: 25 })
      .then((results) => {
        if (!cancelled) {
    ...
    switch (event.key) {
      ...
      case "Enter": {
        ...
        if (selectedIndex >= 0 && results[selectedIndex]) {
          ...
          !mark ? addLink(selectedDoc.path) : updateLink(selectedDoc.path);
                ...
                <SuggestionsMenuItem
                  ...
                  icon={
                    doc.icon ? (
                      <Icon
                        value={doc.icon}
                        initial={doc.initial ?? doc.title.slice(0, 1)}
