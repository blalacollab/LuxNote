...
function useItems({
  ...
  useEffect(() => {
    ...
    const request: Promise<{ embeddable: boolean; reason?: string }> =
      hostAdapter?.checkEmbed
        ? hostAdapter.checkEmbed({ url: singleUrl })
        : Promise.resolve({ embeddable: true });
  ...
  const convertibleToMentionList = pastedText.every((text) => {
    ...
    const mentionType = MentionType.URL;
