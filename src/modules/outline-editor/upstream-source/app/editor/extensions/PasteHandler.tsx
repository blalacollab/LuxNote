          ...
          handlePaste: (view, event: ClipboardEvent) => {
            ...
            if (!this.shiftKey) {
              ...
              if (isUrl(text)) {
                ...
                const hostAdapter = this.editor.props.hostAdapter;

                if (hostAdapter?.resolveInternalLink) {
                  void hostAdapter
                    .resolveInternalLink({ url: text })
                    .then((resolved) => {
                      if (view.isDestroyed) {
                        return;
                      }
                      ...
                      const hasEmoji =
                        determineIconType(resolved.icon ?? null) ===
                        IconType.Emoji;
                      const title = `${hasEmoji ? `${resolved.icon} ` : ""}${
                        resolved.label
                      }`;
                      this.insertLink(
                        `${resolved.path}${resolved.hash ?? ""}`,
                        title
                      );
                    })
                    .catch(() => {
